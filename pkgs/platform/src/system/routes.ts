...
import { CMS, generateAvailableLayouts } from './main'
  ...
  if (p._devmode) {
    ...
    switch (p._action) {
      ...
      case 'batch-edit':
        ...
        return
      case 'reload-layouts':
        reply.send(await generateAvailableLayouts(req.headers['user-agent']))
        return
      ...
      case 'broadcast-reload':
        if (req.body) {
          ...
          const page = system.cache.pages[body.id]
          let url = ''
          if (page && page.slug) {
            url = page.slug
          }
          broadcastHMR({
            type: 'hmr-reload-page',
            id: body.id,
            url,
          })
        }
