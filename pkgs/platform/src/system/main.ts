...
export class CMS {
  ...
  async renderInternal(props: {
    ...
    let layoutPage = system.cache.layout[page.layout_id]
    if (!layoutPage) {
      layoutPage = system.cache.layout['00000']
    }
  ...
  async renderCode(ua: string, layoutHtml: any, childrenHtml: any) {
    ...
    try {
      ...
      log('dev', 'Error Parsing HTML while RenderCode():\n' + e)
    }
  ...
  async renderRoot(props: {
    ...
    if (root) {
      ...
      body = await CMS.updateBodyPack({ body, req, user, ssr, params, id })
  ...
  static updateBodyPack = async (props: {
    body: string
    req: any
    user: any
    id: any
    ...
    const { body, req, user, ssr, params, id } = props
    const initialData = {
      build_id: system.build_id,
      user: JSON.stringify(user.user || { role: 'guest' }),
...
export const generateAvailableLayouts = async (ua: string) => {
  const result = {}
