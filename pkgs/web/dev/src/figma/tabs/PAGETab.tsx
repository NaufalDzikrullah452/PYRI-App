...
import { BaseForm } from '../../../../crud/src/form/BaseForm'
interface ITarget {
  type: 'page' | 'comp'
  id: string
  name: string
...
export const PAGETab = observer(({ frame, node, target, html }: any) => {
  ...
  const reloadList = async () => {
    ...
    let e: any = await sendParent('get-layouts')
    meta.list.layouts = [{}, ...e.layouts] as any

    let pages: any = await api('/__cms/0/list-template')
    runInAction(
      () =>
        (meta.list.pages = [
          {},
    ...
    e = await loadComp({ name: meta.target.name })
    runInAction(() => {
  ...
  if (!meta.target || !meta.html || !meta.init)
    return (
