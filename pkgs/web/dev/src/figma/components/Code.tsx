...
export const Code = observer(({ node, frame, updateIsWrong }: any) => {
          ...
          editor.onDidFocusEditorText(async () => {
            window.figmaSaveCode = async () => {
              ...
              if (res) {
                ...
                const nodeHtml = (await sendParent(
                  'get-node-html',
                  wrap
                )) as any
                nodeHtml.html = await window.babel.prettier(nodeHtml.html)
                if (node.name.indexOf('__.') === 0) {
                  await sendParent('set-node-data', {
                    name: 'elCode',
                    value: nodeHtml.rawHtml,
                  })
                }
                ...
                const html = await window.babel.prettier(
                  await compileFigmaReactElement(
            ...
            window.figmaHtmlForCode = (await sendParent('html-for-code')) as any
          })
...
export const compileFigmaReactElement = async (src: string) => {
  const gen = window.babel.generate
  const trav = window.babel.traverse
  const ast = parse(src, {
    sourceType: 'module',
    plugins: ['jsx'],
  })
  const elements = (await sendParent('reload-elements')) as any

  const scan = () => {
    for (let name of Object.keys(elements)) {
      try {
        elements[name] = parse(elements[name] as any, {
          sourceType: 'module',
          plugins: ['jsx'],
        }).program.body[0]
      } catch (e) {
        console.log(`Failed to parse ${name}`, elements[name])
        console.error(e)
      }
    }
    const pendingFRE = {}
    trav(ast, {
      MemberExpression: (path) => {
        if (
          ...
          if (!elements[name]) {
            pendingFRE[name] = true
          }
          if (elements[name]) {
            if (typeof elements[name] === 'object') {
              delete pendingFRE[name]
              path.replaceWith(elements[name] as any)
            } else {
              pendingFRE[name] = true
            }
          }
  ...
  if (Object.keys(emptyTags).length > 0) {
    alert(`Frame not found: ${Object.keys(emptyTags)}`)
  }

  const res = gen(ast, {}, src)
  return window.babel.prettier(res.code)
}
...
const applyNode = (
  ...
  let lastJSXNode = null as any
  let nodeResult = null as any
  traverse(ast, {
    enter(path) {
      if (path.node && path.node.type === 'JSXElement') {
        ...
        if (path.node.openingElement.name.type === 'JSXIdentifier') {
          if (path.node.openingElement.name.name === node.tag || !node.tag) {
            if (node.name.indexOf('__.') === 0) return
  ...
  if (finalNode && finalNode.type && node.name.indexOf('__.') !== 0) {
...
const parseJSXElement = (fnode: JSXElement, generate) => {
  ...
  if (fnode.type === 'JSXElement') {
    ...
    if (children.length > 0) {
      const childrenource = children.join(' ').trim()
      props.children = childrenource
    }
    code = output.code
  }
  return { props, code }
...
const replaceFirstQuote = (text: string, replace: string) => {
  ...
  if (q) return text.replace(q[0], `${q[0]}${replace} `)
  return ''
}
