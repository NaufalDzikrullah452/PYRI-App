...
export const tailwindMain = (
  ...
  let isLayout = sceneNode[0].name.toLowerCase().indexOf('layout:') === 0
  if (!isLayout) {
    if (parentId && sceneNode[0].id !== parentId) {
      const parent = figma.currentPage.findChild((e) => e.id === parentId)
      if (parent) {
        isLayout = parent.name.toLowerCase().indexOf('layout:') === 0
      }
    }
  }
