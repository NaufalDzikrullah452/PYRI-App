    ...
    case 'get-node-html':
      {
        ...
        if (msg.data.name) {
          ...
          node = frame.findOne((e) => {
            return e.name === msg.data.name
          })
        } else {
          node = figma.currentPage.selection[0]
        }

        if (node && msg.data.wrapCode && msg.data.props) {
    ...
    case 'html-for-code':
      {
        ...
        if (hasLayout) {
          ...
          html = tailwindMain(altFrames, frame.id, false, false, true, node)
        }
    ...
    case 'upd-component':
      {
        ...
        if (frame.type === 'COMPONENT') {
          frame.setPluginData('acceptChildren', findChildren(frame) ? 'y' : 'n')
        }
        sendUI('upd-component', { status: 'OK' })
      }
      break
    case 'reload-elements':
      {
        const node = figma.currentPage.selection[0]
        ...
        frame.setPluginData('elements', '')
        sendUI('reload-elements', getElements(frame))
      }
      break
    case 'set-frame-data':
      {
        ...
        if (x.name === msg.data.page) {
          for (let y of x.children) {
            if (y.type === 'FRAME' && y.name === msg.data.frame) {
              ...
              const html = tailwindMain(altFrames, frame.id, false, false)
  ...
  if (hasLayout) {
    ...
    if (children) {
      ...
      if (converted.length > 0) {
        ...
        finalChild.tagName = 'fragment'
        finalChild.props = {}
        html = tailwindMain([finalChild], frame.id, false, false, false, true)
      }
    }
  } else {
    html = tailwindMain(
...
const getFrameHtml = (frame?: any, replaceNode?: any) => {
  ...
  const html = tailwindMain(altNodes, frame.id, false, false, true, replaceNode)
...
const getNodeHtml = (node: any) => {
  ...
  if (node.id !== frame.id) {
    ...
    rawHtml = tailwindMain(rawNodes, frame.id, false, false, false, false)
  }

  if (html.endsWith(';')) html = html.substr(0, html.length - 1)

  if (rawHtml.endsWith(';')) rawHtml = rawHtml.substr(0, rawHtml.length - 1)
...
const getElements = (target: FrameNode) => {
  if (typeof target === 'object') {
    let result = target.getPluginData('elements') as any
    ...
    target.findAll((e) => {
      if (e.name.indexOf('__.') === 0) {
        const elCode = e.getPluginData('elCode')
        if (elCode) {
          result[e.name] = elCode
        } else {
          result[e.name] = getNodeHtml(e).rawHtml
          e.setPluginData('elCode', result[e.name])
        }
      }
      return false
    })

    target.setPluginData('elements', JSON.stringify(result))
    return result
  }
  return null
}
